...
#ifdef WIN32
#ifdef GROWINGVECTORVM_EXPORTS
#define GROWINGVECTORVM_API __declspec(dllexport)
#else
#define GROWINGVECTORVM_API __declspec(dllimport)
#endif
#else // GROWINGVECTORVM_STATIC
#define GROWINGVECTORVM_API
#endif


namespace ObjectLifecycleHelper
{
...
{
    [[nodiscard]] GROWINGVECTORVM_API size_t CalculateVirtualPageSize(const bool isLargePagesEnabled);

    GROWINGVECTORVM_API void NullifyMemory(void* destination, const size_t size);

    [[nodiscard]] GROWINGVECTORVM_API size_t CalculateInstalledRAM();

    [[nodiscard]] GROWINGVECTORVM_API void* ReserveVirtualMemory(
    ...
    [[nodiscard]] GROWINGVECTORVM_API void* CommitVirtualMemory(
        void*& destination,
        const size_t memorySizeToCommit
        // TODO probably flags should be customizable also
    );

    bool GROWINGVECTORVM_API DecommitVirtualMemory(
        void*& destination,
        const size_t memorySizeToDecommit
    );

    bool GROWINGVECTORVM_API ReleaseVirtualMemory(void* destination);
};
